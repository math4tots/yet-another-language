...
import { parse } from '../lang2/frontend/parser';
import { ParseError } from '../lang2/frontend/ast';
...
//     this.depth--;
//   }
//   visitAwait(n: yal.ast.Await): void {
//     this.indent();
//     this.out += `YIELD`;
//     this.depth++;
//     n.value.accept(this);
...
function stripRanges(obj: any) {
  if (obj && typeof obj === 'object') {
    if (obj instanceof ParseError) {
      // keep
    } else if (Array.isArray(obj)) {
      for (const item of obj) stripRanges(item);
    } else {
      for (const key in obj) {
        if (key === 'range') {
          delete (obj as any)[key];
        } else {
          stripRanges((obj as any)[key]);
        }
      }
    }
  }
